...
describe("Document-Copy-FO", () => {
  ...
  it("ยื่นคำขอคัดสำเนาเอกสาร พร้อม capture screenshot", () => {
    ...
    // cy.contains("ค่าธรรมเนียม").click({ force: true });
    // cy.url().should("include", "/fee-payment");

    // hover ที่เมนูหลัก
    cy.get(".label").contains("บริการและค่าธรรมเนียม").trigger("mouseover");

    // รอให้ submenu โผล่ขึ้นมา
    cy.get('a[href="/fee-payment/"]', { timeout: 10000 }).should("be.visible");

    // คลิก submenu
    cy.get('a[href="/fee-payment/"]').click();

    // ตรวจสอบว่าไปหน้าที่ถูกต้อง
    cy.url().should("include", "/fee-payment");

    // เอาเมาส์ไป hover menu
    // cy.contains(".label", "บริการและค่าธรรมเนียม").trigger("mouseover");
    // cy.contains(".label", "ค่าธรรมเนียม")
    //   .parents("a")
    //   .first()
    //   .click({ force: true });
